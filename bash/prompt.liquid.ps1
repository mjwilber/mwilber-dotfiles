...
LP_ENABLE_PROJ_SPEC="${LP_ENABLE_PROJ_SPEC:-0}"

_lp_local_proj_spec()
{
    [[ "$LP_ENABLE_PROJ_SPEC" != 1 ]] || [[ "_${PRJ}_" = "__" ]] && return
    echo -n "${LP_COLOR_PROJ_SPEC}"
    echo -n "${LP_BRACKET_OPEN}"
    echo -n "${PRJ:--}:${PBRANCH:--}"
    echo -n "${LP_BRACKET_CLOSE}${NO_COL}"
    echo -n "${NO_COL}"
}

# LP_PROJ_SPEC="" #$(_lp_sr "$(_lp_local_proj_spec)")"
LP_PROJ_SPEC="$(_lp_sr "$(_lp_local_proj_spec)")"
...
# is set.
LP_PS1="${LP_PS1_PREFIX}${LP_PROJ_SPEC}${LP_VCS}"

# add time, jobs, load and battery
LP_PS1="${LP_PS1}${LP_PS1_PREFIX}${LP_TIME}${LP_BATT}${LP_LOAD}${LP_JOBS}"
